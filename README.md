...
**EWA AI String Assistant for Loco Translate** is a powerful, production-grade WordPress add-on for [Loco Translate](https://wordpress.org/plugins/loco-translate/). It introduces seamless AI-assisted translation directly into the Loco Translate editor using **OpenRouter**, **Ollama**, or any custom **OpenAI-compatible** endpoint.

Developed by **[Error Web Agency](https://error.bg/)**.
...
2. Upload the `ewa-ai-string-assistant-for-loco-translate.zip` file.
3. Click **Install Now** and then **Activate**.

### Manual Installation
1. Upload the `ewa-ai-string-assistant-for-loco-translate` directory to `/wp-content/plugins/`.
...
1. Navigate to **Settings → EWA AI String Assistant** in the WordPress admin panel.
...
## 🌐 External Services & Data Disclosures

This plugin connects to external AI services only when an administrator configures a provider and explicitly initiates an action (such as loading models, testing a connection, or running translation):
...
* **Ollama**: Transmits translation requests to the configured local or remote Ollama server. For local instances (`http://localhost:11434`), data remains strictly local. See [Ollama Privacy](https://ollama.com/privacy).
...
## ⚠️ Terms of Use & Disclaimer (Условия за ползване и отказ от отговорност)
...
Under no circumstances shall **Error Web Agency**, its developers, or contributors be held liable for any direct, indirect, incidental, special, consequential, or punitive damages, including but not limited to:
* **Data or file loss**: Corrupted, damaged, or overwritten translation files (`.po` / `.mo`).
* **Translation quality**: Inaccuracies, grammatical errors, mistranslations, or hallucinations generated by third-party AI models.
* **Third-party costs**: Any API usage fees, token billing overages, or service interruptions from external AI providers (OpenRouter, OpenAI, etc.).
* **Business disruption**: Lost profits, loss of clients, or business interruption arising out of the use or inability to use this plugin.

**User Responsibility:**
1. **Mandatory Backups**: You are solely responsible for backing up your website and all `.po`/`.mo` translation files before initiating automated translations.
2. **Review**: You must independently review and verify all machine-generated translations before publishing them to a live/production site.
3. **API Keys**: You are responsible for safeguarding your third-party API credentials and configuring spending limits with your provider.

---

### Български (Условия за ползване)
Този плъгин се предоставя във вида, в който е (**„AS IS“**), без каквито и да е гаранции, преки или косвени. 

**Error Web Agency** и неговите разработчици не носят отговорност за каквито и да е преки, косвени, случайни или последващи вреди, включително, но не само:
* Презаписване, повреждане или загуба на преводни файлове (`.po` и `.mo`).
* Неточности, фактически грешки или халюцинации в генерираните от AI преводи.
* Начислени разходи и такси от външни доставчици на AI услуги (OpenRouter, платени API и др.).
* Прекъсване на дейността, пропуснати ползи или вреди върху бизнеса, породени от използването или невъзможността за използване на плъгина.

**Отговорност на потребителя:**
1. **Задължителен бекъп:** Потребителят е длъжен да направи резервно копие (backup) на своите `.po` и `.mo` файлове преди стартиране на масови преводи.
2. **Преглед на съдържанието:** Машинните преводи трябва да бъдат прегледани от администратора преди публикуването им в реална среда.
3. **API управление:** Потребителят носи пълна отговорност за своите API ключове и финансови лимити при външните доставчици.
...
* **Author**: [Error Web Agency](https://error.bg/)
* **Contributors**: errorwebagency
* **Lead Developer**: K2D
* **Repository**: [github.com/error-agency/ewa-ai-string-assistant-for-loco-translate](https://github.com/error-agency/ewa-ai-string-assistant-for-loco-translate)
